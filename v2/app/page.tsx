...
// V2 with integrated vehicle search
"use client";
import React, { useState } from "react";
import { RefreshCcw } from "lucide-react";
import { CommuteFormV2 } from "@/components/CommuteFormV2";
...
export default function Page() {
  // Initialize with default vehicles and allow dynamic additions
  ...
  // Handler to add custom vehicle from API search
  const handleCustomVehicleAdd = (vehicle: Vehicle, slot: 'current' | 'new') => {
    ...
    // Set the appropriate vehicle ID based on slot
    if (slot === 'current') {
      setValues(v => ({ ...v, currentVehicleId: vehicle.id }));
    } else {
      setValues(v => ({ ...v, newVehicleId: vehicle.id }));
    }
  };
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="cyber-card p-6 space-y-6">
          <CommuteFormV2 
            ...
            onCustomVehicleAdd={handleCustomVehicleAdd}
          />
          <button 
            onClick={run} 
            disabled={loading} 
